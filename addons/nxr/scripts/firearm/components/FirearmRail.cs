	...
	public override void _Process(double delta)
	{
		...
		foreach (InteractableSnapZone zone in _zones)
		{
			...
			if (zone.SnappedInteractable != null && zone.SnappedInteractable.IsInsideTree())
			{
				...
				if (zone.SnappedInteractable.PrimaryGrab.Interactor != null)
				{
					Vector3 newPos = parent.ToLocal(zone.SnappedInteractable.GetPrimaryController().GlobalPosition);
					...
					{
						zone.SnappedInteractable.GetPrimaryController().Pulse(1.0, 0.05, 0.1);
					}
